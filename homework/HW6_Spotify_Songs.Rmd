...
```{r}
spotify_2020 <- read.csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/spotify_2020.csv")
```
...
Write a few sentences about the distribution of danceability scores in the Spotify dataset. Is it a normal distribution, or is it skewed in one direction? What does the mean and one standard distribution tell us about the songs in the data set? Draft a short story pitch from this data.

The danceability scores are not normally distributed. Instead, it looks like it is skewed to the left. The mean and the standard distribution is telling be that there are more songs on spotify's 2020 dataset that are considered to have a higher, or above average danceability. 
