...
I would definitely say that this is a useful model based on the correlation analysis. With a multiple r-squared value of 0.632 which is definitely significant, one can see that between the three independent variables, there is definitely a significant impact between the three and winning football games. 
...
Points scored has the most impact on wins with a p-value of 0.00665. This stands below the threshold of 0.05, meaning that it is significant. Takeaways are sort of significant More on the borderline aspect with a p value of 0.07928. This stands slightly above that 0.05 aspect. Finally, yards gained aren't really that close to being significant with a p-value of 0.34332. This stands very far away from 0.05 and proves not to be a factor of wins. 

## Using the Model

Without using the words "multiple regression model", describe how you could use this model to cover a team or game.

You can easily talk about how these three factors help see if it helps teams win games. It can help you rule things out. For example using my team the Atlanta Falcons, one might say that the reason they win games is because of their points scored. Or, on the other hand, the Falcons might score a lot of points per game but sit at a very poor 4-13 record. This would be very backed up by the data. 