...
ANSWER: Looking at the scatter plot we created, the relationship between FARMS percentage and math test scores are very average, if not a little positive. It does not like a higher FARMS percentage leads to that much of higher test scores. There is a lot of data that is above and below the line of best fit. 
...
ANSWER: I would definitely say that this model is worth using! Being able to calculate the average test scores for a school can definitely be useful in multiple degrees. However, what is does tell us about the nature of the relationship between FARMS percentage and math scores is that there really isn't much impact or influence between the two. The FARMS percentage doesn't really have a significance at all to the math test scores. 
...
ANSWER: Some of the largest differences between the predicted result and the actual math score are 	
WHITEMARSH EL SCH, that saw a residual of -34.9. The next difference that was very large was 	
GULPH ELEMENTARY SCH that has a residual of -31.34. Then the last school with a really large residual comes from CROOKED BILLET EL SCH with a residual of 31.11. 
...
ANSWER: Conclusions I would draw from this model's results is the fact that there is really no comparison or influence between the tests we just made. The p-value showed us that there was no direct comparison between the data sets, so there is no real conclusion that we can draw from this data. This would influence a story because if we would use data that we got from this test, there would be a chance of the predictions being completely wrong or off. 
